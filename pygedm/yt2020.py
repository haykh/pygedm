...
c_scaling = (
    U_Msun / U_pc**3 * pow(10, -18)
)  # scaling factor [10^{12} M_sun/kpc^3] -> [g/cm^3]
...
f_NFW = np.log(1.0 + c_NFW) - c_NFW / (
    1.0 + c_NFW
)  # f(x)=np.log(1.0+x)-x/(1.0+x) and x = c_NFW
rho_s = (
    M_vir / 4.0 / np.pi / r_s ** (3.0) / f_NFW
)  # NFW scale density [10^{12} M_sun/kpc^3]

### calculate n_0^{sphe} (eq.[3]) by normalization (eq.[4]) ###

r_array = np.linspace(r_s / 100, r_vir, 100000)  # r = 0-r_vir [kpc]
Upsilon = (
    4 * np.pi * 10 * U_G * U_Msun / U_pc * U_mp / U_eV * r_s**2 * rho_s * mu / T
)  # Upsilon appearing in right-hand side of eq.(3)
...
def s_max(
    l, b
):  # maximum integration limit corresponsing to r = r_vir as function of sky coordinates [kpc]
    ...
    """
    return D_sun * cos(b) * cos(l) + sqrt(
        D_sun**2 * (cos(b) ** 2 * cos(l) ** 2 - 1) + (1.0 * r_vir) ** 2
    )
...
def ne_sphe(l, b, s):
    ...
    return n_0_sphe * np.exp(
        -Upsilon * (1 - np.log(1 + r / r_s) * r_s / r)
    )  # [cm^{-3}]
